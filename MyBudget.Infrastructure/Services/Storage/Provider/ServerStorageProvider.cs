...
namespace MyBudget.Infrastructure.Services.Storage.Provider
{
    public class ServerStorageProvider : IStorageProvider
    {
        ...
        {
            //TODO - replace on implementation (added for tests)--
            return _storage.ContainsKey(key) ? ValueTask.FromResult(_storage[key]) : ValueTask.FromResult(string.Empty);
            //----------------------------------------------------
